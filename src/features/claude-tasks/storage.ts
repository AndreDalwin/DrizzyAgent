...
export function getTaskDir(config: Partial<DrizzyAgentConfig> = {}): string {
  const tasksConfig = config.coder?.tasks
...
export function resolveTaskListId(config: Partial<DrizzyAgentConfig> = {}): string {
  ...
  const configId = config.coder?.tasks?.task_list_id?.trim()
...
export function listTaskFiles(config: Partial<DrizzyAgentConfig> = {}): string[] {
