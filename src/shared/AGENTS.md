...
Cross-cutting utilities used throughout the plugin. Barrel-exported from `index.ts`. Logger writes to `/tmp/drizzy-agent.log`.
...
- `agent-names.ts`: Old agent names → new (e.g., `junior` → `coder-junior`)
...
|---------|-------------|---------|
| `logger.ts` | 62 | `/tmp/drizzy-agent.log` |
