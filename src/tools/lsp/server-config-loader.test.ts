...
describe("loadJsonFile", () => {
  ...
  it("discovers JSONC-only user config (drizzy-agent.jsonc)", () => {
  ...
  "lsp": {
    "user-jsonc": {
      ...
      const userPath = join(tempBase, "drizzy-agent.jsonc")
  ...
  it("discovers JSONC-only project config (.opencode/drizzy-agent.jsonc)", () => {
  ...
  "lsp": {
    "project-jsonc": {
      ...
      const projectPath = join(tempProject, ".opencode", "drizzy-agent.jsonc")
  ...
  "lsp": {
    "conflict": {
      ...
      writeFileSync(join(tempBase, "drizzy-agent.json"), jsonContent, "utf-8")
      writeFileSync(join(tempBase, "drizzy-agent.jsonc"), jsoncContent, "utf-8")
