...
bunDescribe("sendSyncPrompt", () => {
  ...
  bunTest("applies agent tool restrictions for explore agent", async () => {
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.tools.call_drizzy_agent).toBe(false)
  })
  ...
  bunTest("does not restrict call_drizzy_agent for coder agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.tools.call_drizzy_agent).toBe(true)
  })
