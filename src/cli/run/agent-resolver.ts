...
const CORE_AGENT_ORDER = ["coder", "gptcoder", "planner", "orchestrator", "researcher"] as const
...
const isAgentDisabled = (agentConfigKey: string, config: DrizzyAgentConfig): boolean => {
  const lowered = agentConfigKey.toLowerCase()
  if (lowered === DEFAULT_AGENT && config.coder_agent?.disabled === true) {
    return true
  }
...
const pickFallbackAgent = (config: DrizzyAgentConfig): CoreAgentKey => {
