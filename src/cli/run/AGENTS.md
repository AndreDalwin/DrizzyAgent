...
37 files. Powers the `drizzy-agent run <message>` command. Connects to OpenCode server, creates/resumes sessions, streams events, and polls for completion.
...
runner.ts
  ...
  3. agent-resolver.ts → Flag → env → config → Coder
...
| `agent-resolver.ts` | Resolve agent: `--agent` flag → `OPENCODE_AGENT` env → config → Coder |
