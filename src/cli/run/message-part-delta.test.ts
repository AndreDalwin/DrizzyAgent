...
describe("message.part.delta handling", () => {
  ...
  it("renders agent header using profile hex color when available", () => {
    ...
    const ctx = createMockContext("ses_main")
    const state = createEventState()
    state.agentColorsByName["Coder"] = "#00CED1"
    const stdoutSpy = spyOn(process.stdout, "write").mockImplementation(() => true)
    const payload: EventPayload = {
    ...
    const events: EventPayload[] = [
      {
        type: "message.updated",
        properties: {
          info: { sessionID: "ses_main", role: "assistant", agent: "Coder", modelID: "claude-opus-4-6" },
        },
      },
      {
        type: "message.part.updated",
    ...
    const events: EventPayload[] = [
      {
        type: "message.updated",
        properties: {
          info: { id: "msg_assistant", sessionID: "ses_main", role: "assistant", agent: "Coder", modelID: "claude-opus-4-6" },
        },
      },
      {
        type: "message.part.updated",
    ...
    const events: EventPayload[] = [
      {
        type: "message.updated",
        properties: {
          info: { id: "msg_assistant", sessionID: "ses_main", role: "assistant", agent: "Coder", modelID: "claude-opus-4-6", variant: "max" },
        },
      },
      {
        type: "message.part.delta",
    ...
    const events: EventPayload[] = [
      {
        type: "message.updated",
        properties: {
          info: { id: "msg_user", sessionID: "ses_main", role: "user", agent: "Coder", modelID: "claude-opus-4-6" },
        },
      },
      {
        type: "message.part.updated",
