...
| `agent-priority-order.ts` | ~30 | coder, GPTCoder, planner, orchestrator first |
...
|-------|---------|--------|
| Librarian | grep_app_* | — |
| Orchestrator, Coder, Planner | task, task_*, teammate | — |
| GPTCoder | task | — |
...
```
User (~/.config/opencode/drizzy-agent.jsonc)
  ↓ deepMerge
Project (.opencode/drizzy-agent.jsonc)
  ↓ Zod defaults
Final Config
```
