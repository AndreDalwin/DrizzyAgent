...
24 schema files composing `DrizzyAgentConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── drizzy-agent-config.ts    # ROOT: DrizzyAgentConfigSchema (composes all below)
...
├── coder.ts                 # CoderConfigSchema (task system)
├── coder-agent.ts           # CoderAgentConfigSchema
...
2. Add field to `drizzy-agent-config.ts` root schema
