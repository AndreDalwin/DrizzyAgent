...
| File | Purpose |
|------|---------|
| `index.ts` | Plugin entry, exports `DrizzyAgentPlugin` |
...
loadPluginConfig(directory, ctx)
  1. User: ~/.config/opencode/drizzy-agent.jsonc
  2. Project: .opencode/drizzy-agent.jsonc
...
createHooks()
  ...
  │   ├─ createSessionHooks()     # 23: contextWindowMonitor, thinkMode, ralphLoop, modelFallback, runtimeFallback, noCoderGpt, noGptcoderNonGpt, anthropicEffort, intentGate...
  ...
  ├─→ createContinuationHooks()   # 7: todoContinuationEnforcer, orchestrator, stopContinuationGuard, ralphLoopActivator...
