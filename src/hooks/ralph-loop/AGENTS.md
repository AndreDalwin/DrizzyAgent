...
/ralph-loop → startLoop(sessionID, prompt, options)
  → loopState.startLoop() → persists state to .drizzy/ralph-loop.local.md
...
| `storage.ts` | Read/write `.drizzy/ralph-loop.local.md` state file |
...
```
.drizzy/ralph-loop.local.md  (gitignored)
