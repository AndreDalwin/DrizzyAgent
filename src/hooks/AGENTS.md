...
Self-referential dev loop via `/ralph-loop` command. State persisted in `.drizzy/ralph-loop.local.md`. Detects `<promise>DONE</promise>` in AI output. Max 100 iterations default.
