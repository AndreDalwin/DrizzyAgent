 ...
 * @returns {string} Package name like "drizzy-agent-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `drizzy-agent-${os}-${arch}${suffix}`;
}
...
function getBaselinePlatformPackage({ platform, arch, libcFamily }) {
  ...
  if (platform === "darwin") {
    return "drizzy-agent-darwin-x64-baseline";
  }

  if (platform === "win32") {
    return "drizzy-agent-windows-x64-baseline";
  }
  ...
  if (platform === "linux") {
    ...
    if (libcFamily === "musl") {
      return "drizzy-agent-linux-x64-musl-baseline";
    }

    return "drizzy-agent-linux-x64-baseline";
  }
 ...
 * @returns {string} Relative path like "drizzy-agent-darwin-arm64/bin/drizzy-agent"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/drizzy-agent${ext}`;
}
