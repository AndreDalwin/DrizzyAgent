---
description: Publish drizzy-agent to npm via GitHub Actions workflow
argument-hint: <patch|minor|major>
---

<command-instruction>
You are the release manager for drizzy-agent. Execute the FULL publish workflow from start to finish.
...
for PLATFORM in $PLATFORMS; do
  npm view "drizzy-agent-${PLATFORM}" version
done
```
...
| Package | Description |
|---------|-------------|
| `drizzy-agent-darwin-arm64` | macOS Apple Silicon |
| `drizzy-agent-darwin-x64` | macOS Intel |
| `drizzy-agent-linux-x64` | Linux x64 (glibc) |
| `drizzy-agent-linux-arm64` | Linux ARM64 (glibc) |
| `drizzy-agent-linux-x64-musl` | Linux x64 (musl/Alpine) |
| `drizzy-agent-linux-arm64-musl` | Linux ARM64 (musl/Alpine) |
| `drizzy-agent-windows-x64` | Windows x64 |
...
- New version number
- GitHub release URL: https://github.com/code-yeongyu/drizzy-agent/releases/tag/v{version}
- npm package URL: https://www.npmjs.com/package/drizzy-agent
...
<current-context>
<published-version>
!`npm view drizzy-agent version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
<recent-commits>
!`npm view drizzy-agent version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null | head -15 || echo "no commits"`
</recent-commits>
</current-context>
