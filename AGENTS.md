# DrizzyAgent — O P E N C O D E Plugin

**Generated:** 2026-03-06 | **Commit:** 7fe44024 | **Branch:** dev

## OVERVIEW

OpenCode plugin (npm: `drizzy-agent`) that extends Claude Code (OpenCode fork) with multi-agent orchestration, 46 lifecycle hooks, 26 tools, skill/command/MCP systems, and Claude Code compatibility. 1268 TypeScript files, 160k LOC.
...
| Modify config schema | `src/config/schema/` + update root schema | Zod v4, add to DrizzyAgentConfigSchema |
| Add new category | `src/tools/delegate-task/constants.ts` | DEFAULT_CATEGORIES + CATEGORY_MODEL_REQUIREMENTS |

## MULTI-LEVEL CONFIG

```
Project (.opencode/drizzy-agent.jsonc)  →  User (~/.config/opencode/drizzy-agent.jsonc)  →  Defaults
```
...
bunx drizzy-agent install # Interactive setup
bunx drizzy-agent doctor  # Health diagnostics
bunx drizzy-agent run     # Non-interactive session
```
...
- Logger writes to `/tmp/drizzy-agent.log` — check there for debugging
